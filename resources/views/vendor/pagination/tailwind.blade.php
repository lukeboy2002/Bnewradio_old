            ...
            <div>
                <p class="text-sm text-gray-700 leading-5">
                    ...
                    @if ($paginator->firstItem())
                        <span class="font-bold text-red-700">{{ $paginator->firstItem() }}</span>
                        {!! __('to') !!}
                        <span class="font-bold text-red-700">{{ $paginator->lastItem() }}</span>
                    @else
                        {{ $paginator->count() }}
                    @endif
                    {!! __('of') !!}
                    <span class="font-bold text-red-700">{{ $paginator->total() }}</span>
                    {!! __('results') !!}
                </p>
            </div>
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium font-bold text-red-700 bg-red-100 border border-gray-300 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
