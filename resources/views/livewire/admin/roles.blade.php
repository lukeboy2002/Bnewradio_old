    ...
    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
        <div class="bg-gray-50 border border-gray-200 flex justify-between items-center py-3 px-6 mb-6 sm:rounded-lg">
            <div class="flex items-center text-red-700">
                ...
                <h3 class="text-md font-bold">Role Management</h3>
            </div>

            <div>
                <x-links.default_btn href="{{ route('admin.roles.create') }}" class="bg-red-700"><i class="fa-solid fa-folder-plus mr-2"></i>Add Role</x-links.default_btn>
            </div>
        </div>
                ...
                @forelse($roles as $role)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-gray-500">
                            <div class="text-sm font-medium text-gray-500">{{ $role->name }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                            <x-links.default_btn href="{{ route('admin.roles.edit' , $role->id) }}" class="bg-gray-600">Edit</x-links.default_btn>
                            <x-buttons.default wire:click="deleteId({{ $role->id }})" class="bg-red-700">Delete</x-buttons.default>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5">There are no Roles yet</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
